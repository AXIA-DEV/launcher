...
export async function getAllychainIdFromSpec(
	bin: string,
	chain?: string
): Promise<number> {
...
// Export the genesis wasm for a allychain and return it as a hex encoded string starting with 0x.
// Used for registering the allychain on the relay chain.
export async function exportGenesisWasm(
	...
	let opts = { maxBuffer: 20 * 1024 * 1024 };
...
export function startCollator(
	...
	return new Promise<void>(function (resolve) {
		...
		// if (!skip_id_arg) {
		// 	args.push("--allychain-id=" + id);
		// 	console.log(`Added --allychain-id=${id}`);
		// }
		if (onlyOneAllychainNode) {
		...
		let flags_allychain = null;
		let split_index = flags ? flags.findIndex((value) => value == "--") : -1;

		if (split_index < 0) {
			flags_allychain = flags;
		} else {
			flags_allychain = flags ? flags.slice(0, split_index) : null;
			flags_collator = flags ? flags.slice(split_index + 1) : null;
		}

		if (flags_allychain) {
			// Add any additional flags to the CLI
			args = args.concat(flags_allychain);
			console.log(`Added ${flags_allychain} to allychain`);
		}
