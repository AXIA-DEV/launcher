...
`allychains` is an array of objects that consists of:

- `bin`: The path of the [collator node
  binary](https://github.com/substrate-developer-hub/substrate-allychain-template) used to create
  blocks for your allychain. For example
  `<path/to/substrate-allychain-template>/target/release/polkadot-collator`.
- `id`: The id to assign to this allychain. Must be unique.
- `wsPort`: The websocket port for this node.
- `port`: The TCP port for this node.
...
This is similar to `allychains` but for "simple" collators like the adder-collator, a very simple
...
Open HRMP channels between the specified allychains so that it's possible to send messages between
...
This tool just automates the steps needed to spin up multiple relay chain nodes and allychain nodes
...
- [`child_process`](https://nodejs.org/api/child_process.html) is used to execute commands on your
  node:
  - We build a fresh chain spec using the `chain` allymeter specified in your config.
    - Includes the authorities you specified.
    - Includes changes to the `paras`.
    - Includes allychains you have added.
  ...
  - We spawn new node instances using the information provided in your config. Each node produces a
    ...
    # or
    tail -f 9988.log # Collator for Allychain ID 200 on wsPort 9988
    ```
