...
const rootAccount = `panda dose welcome ostrich brief pull lawn table arrest worth ranch faculty//Sankar//stash`;
// const rootAccount = '//Alice';
...
// Get the genesis header of a node. Used for registering a allychain on the relay chain.
...
// Submit an extrinsic to the relay chain to register a allychain.
// Uses the Alice account which is known to be Sudo for the relay chain.
export async function registerAllychain(
	api: ApiPromise,
	id: string,
	wasm: string,
	header: string,
	...
	return new Promise<void>(async (resolvePromise, reject) => {
		...
		const alice = keyring.addFromUri(rootAccount);
		...
		let genesis = api.createType("AllyGenesisArgs", allyGenesisArgs);

		const nonce = Number((await api.query.system.account(alice.address)).nonce);

		console.log(
			`--- Submitting extrinsic to register allychain ${id}. (nonce: ${nonce}) ---`
		);
		const unsub = await api.tx.sudo
			.sudo(api.tx.parasSudoWrapper.sudoScheduleAllyInitialize(id, genesis))
...
export async function extendLeasePeriod(
	...
	console.log(`Extending Allychain ${id} lease period`);
	return new Promise<void>(async (resolvePromise, reject) => {
		await cryptoWaitReady();
		...
		console.log(
			`--- Submitting extrinsic to extend allychain ${id} lease period to ${period_count} days. (nonce: ${nonce}) ---`
		);
		const unsub = await api.tx.sudo
			.sudo(api.tx.slots.forceLease(id, alice.address, 1, 0, period_count))
			.signAndSend(alice, { nonce: nonce, era: 0 }, (result) => {
				console.log(`Current status is ${result.status}`);
				if (result.status.isInBlock) {
					console.log(
