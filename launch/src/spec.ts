...
export async function addAuthority(spec: string, name: string) {
	...
	let key = [
		...
		sr_stash.address,
		{
			...
			allychain_validator: sr_account.address,
			authority_discovery: sr_account.address,
			ally_validator: sr_account.address,
			ally_assignment: sr_account.address,
...
// Add allychains to the chain spec at genesis.
export async function addGenesisAllychain(
	spec: string,
	ally_id: string,
	head: string,
	wasm: string,
	...
	// For retro-compatibility with substrate pre Polkadot 0.9.5
	else if (runtimeConfig.allychainsAllys) {
		paras = runtimeConfig.allychainsAllys.paras;
	}
	if (paras) {
		let new_ally = [
			parseInt(ally_id),
		...
		let data = JSON.stringify(chainSpec, null, 2);
		fs.writeFileSync(spec, data);
		console.log(`  ✓ Added Genesis Allychain ${ally_id}`);
	} else {
...
export async function addGenesisHrmpChannel(
	...
	// For retro-compatibility with substrate pre Polkadot 0.9.5
	else if (runtimeConfig.allychainsHrmp) {
		hrmp = runtimeConfig.allychainsHrmp;
	}
